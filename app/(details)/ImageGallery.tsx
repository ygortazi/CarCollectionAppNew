...
function validateImageUri(uri?: string): boolean {
    return Boolean(uri && (uri.startsWith('file://') || uri.startsWith('http://') || uri.startsWith('https://')));
}

// In getImageDimensions function
async function getImageDimensions(uri: string): Promise<ImageDimensions> {
    if (!validateImageUri(uri)) {
        return Promise.reject(new Error('Invalid image URI'));
    }
